#ifndef _AVLNode_hpp_
#define _AVLNode_hpp_
...
public:
	...
	void left(avlNode *v);//把当前结点的左指针修改为函数的参数
...
template <class T> 
avlNode<T>::avlNode(T val,avlNode<T> *left,avlNode<T> *right,int bf=0)
{
	value=val;
	leftptr=left;
	rightptr=right;
...
// template <class T>
// avlNode<T>* avlNode<T>::copy()const
// {
// 	avlNode<T> *nl,*nr;
...
template <class T> 
void avlNode<T>::left(avlNode<T>* v)
{
	leftptr=v;
}
...
template <class T> 
void avlNode<T>::right(avlNode<T>* v)
{
	rightptr=v;
}
...
template <class T> 
int avlNode<T>::add(avlNode<T>* &rp,T val)
{
	if (val<value)
	{//左子树插入
		if (rp->left()==NULL)
		...
		{//原来已经倾斜，左边失衡，需要做平衡处理
			if (rp->left()->bf<0)  //插入在左侧，单旋转
				...
				rp = LR_doubleRotation();	//插入在右侧，双旋转
			return 0;
		}
...
//删除以当前结点的为根的avl树中的val结点
template <class T> 
avlNode<T>* avlNode<T>::remove(T val,avlNode<T>* &waste,int &flag)
{
	if (val==value)
	{
		waste=this;
...
template <class T> 
avlNode<T>* avlNode<T>::removeLeftmostElement(avlNode<T>* &childptr,int &flag)
{//flag 表示子树高度是否变化
...
template <class T> 
avlNode<T>* avlNode<T>::findNodeValue(T val)
{
	if (val==value)
	{
		return this;
...
template <class T> 
void avlNode<T>::preorderview(avlNode<T> *current,int i)
{
	i++;//层计数器
	if (current)
	{
...
template <class T>
avlNode<T>* avlNode<T>::LL_singleRotation()
{
	avlNode<T> *p;
	p=left();
	left(p->right());
...
template <class T> 
avlNode<T>* avlNode<T>::LR_doubleRotation()
{
	avlNode<T> *p,*q;
	q=left();
	p=q->right();
...
template <class T> 
avlNode<T>* avlNode<T>::RR_singleRotation()
{
	avlNode<T> *p;
	p=right();
	right(p->left());
...
template <class T> 
avlNode<T>* avlNode<T>::RL_doubleRotation()
{
	avlNode<T> *p,*q;
	q=right();
	p=q->left();
